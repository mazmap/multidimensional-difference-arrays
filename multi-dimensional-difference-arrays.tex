...
\begin{frame}{Who am I?}
    \begin{itemize}[<+->] 
        \item Matteo Mertz 
        \item Finishing my mathematics bachelor at TUM 
        \item Currently writing my bachelors thesis under the supervision of Prof. Bauer and Matthias Söls on the \emph{Efficient computation of Euler characteristic profiles (of cubical filtrations of digital images)}
    \end{itemize}
\end{frame}
...
\begin{frame}[fragile]{One-dimensional difference arrays}
    \begin{itemize}[<+->]
        ...
        \item The difference array algorithm works as the cumulative sum distributes over the markers of multiple queries: 
            \begin{equation*}
                \Sigma\texttt{d}[j] = \sum_{l=1}^j \texttt{d}[l] = \sum_{l=1}^j \sum_{i=1}^L \texttt{d}_i[l] = \sum_{i=1}^L \sum_{l=1}^j \texttt{d}_i[l] = \sum_{i=1}^L \Sigma \texttt{d}_i 
            \end{equation*}
...
\begin{frame}[fragile]
    ...
    \begin{tikzpicture}[remember picture, overlay]
        \node [
            ...
            \textcolor{ScreenshotBlue}{\textbf{Runtime:}} 
            The runtime of the outer for-loop lies in \( \mathcal{O}\left( \left|\prod_{\substack{i=1\\i\neq j}}^d \llbracket n_i, m_i \rrbracket\right| \cdot \left|\llbracket n_j+1,m_j \rrbracket\right| \right) = \mathcal{O}(|D|) \) \\
...
\begin{frame}[fragile]
    ...
    \begin{tikzpicture}[remember picture, overlay]
        \node [
            ...
            \textcolor{ScreenshotBlue}{\textbf{Runtime:}} \( \mathcal{O}(N) \)
        };
    \end{tikzpicture}
    }
\end{frame}
...
\begin{frame}
	...
	% 3. Draw Axis Ticks and Labels
	% X-axis ticks (at integer coordinates)
	\draw[thick] (2, -0.1) -- (2, 0.1) node[below=5pt] {$n_1$};
	\draw[thick] (6, -0.1) -- (6, 0.1) node[below=5pt] {$m_1$};

	% Y-axis ticks (at integer coordinates)
	\draw[thick] (-0.1, 2) -- (0.1, 2) node[left=5pt] {$n_2$};
	\draw[thick] (-0.1, 4) -- (0.1, 4) node[left=5pt] {$m_2$};
...
\begin{frame}
    ...
    \node[Green, anchor=center] at (4,3) {$+$};
...
\begin{frame}
    ...
    \node[ScreenshotBlue, anchor=center] at (4,6) {$-$};
