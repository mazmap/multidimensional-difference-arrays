...
\begin{frame}[fragile]{One-dimensional difference arrays}
    ...
    \begin{column}<3->{0.50\textwidth}
		...
		\drawQueryBlock{Cumulative Sum}{-2*\rowBlockGap - \rowBlockGap/2}
...
\begin{frame}[fragile]{One-dimensional difference arrays}
    Imagine placing the markers as forming separate difference arrays \( \texttt{d}_1, \dots, \texttt{d}_L \), that is 
    \begin{equation*}
    ...
    The cumulative sum \( \Sigma\texttt{d} \) can be computed in \( \mathcal{O}(N) \) by reusing previous results: 
    \begin{equation*}
...
\begin{frame}
    \begin{definition*}{constant update queries}
        \begin{itemize}[<+->]
            \item A discrete map \( q : D \to \mathbb{R} \) which is \emph{constant} on a subset \( D_q = \prod_{i=1}^d \llbracket a_i, b_i \rrbracket \subseteq D \) is called \emph{constant update query}
            \item In other words, \( q = C \cdot \mathbb{1}_{D_q} \) with \( C \in \mathbb{R} \) (the \emph{update value} of \( q \))
            \item \( |D_q| \) is the \emph{size} of \( q \) 
        \end{itemize}
    \end{definition*}
...
\begin{frame}[fragile]{A naive algorithm}
    ...
    \begin{tikzpicture}[remember picture, overlay]
        \node [
            ...
            \textcolor{ScreenshotBlue}{\textbf{Runtime:}} With \( N := \card(D) \), \\
            Non-situ: \( \mathcal{O}(N + L\cdot N) = \mathcal{O}(L \cdot N) \) \\
            In-situ: \( \mathcal{O}(L \cdot N) \) 
        };
    \end{tikzpicture}
    }
\end{frame}
...
\begin{frame}{Properties of backward differences and prefix sums}
    \begin{itemize}[<+->]
        \item It is not hard to show that backward differences and prefix sums commute, that is for every \( i \) and \( j \) we have 
            \begin{equation*}
                \Sigma_i \circ \Delta_j = \Delta_j \circ \Sigma_i.
            \end{equation*}
        \item In particular, \( \Sigma \circ \Delta = (\Sigma_1 \circ \Delta_1) \circ \cdots \circ (\Sigma_d \circ \Delta_d) =  \Delta \circ \Sigma \). 
        \item The \( i \)-th backward difference \( \Delta_i \) and the \( i \)-th prefix sum \( \Sigma_i \) are inverse to each other for all \( i \), that is for every discrete map \( f \) we have 
            \begin{equation*}
                \Delta_i \circ \Sigma_i(f) = f. 
            \end{equation*}
...
\begin{frame}[fragile]
    ...
    \begin{tikzpicture}[remember picture, overlay]
        \node [
            ...
            \textcolor{ScreenshotBlue}{\textbf{Runtime:}} \( \mathcal{O}(L + N + N + N) = \mathcal{O}(L+N) \)
        };
    \end{tikzpicture}
\end{frame}
